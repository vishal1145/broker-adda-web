...
const Blogs = ({ data = { subtitle: "", title: "", button: "", items: [] } }: { data: BlogsData }) => {
  ...
  return (
    <section className="bg-white py-8 md:py-16">
      ...
      {/* Blog Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-4 lg:gap-8 mx-auto">
        {(data.items || []).map((blog, index) => (
          <div
            key={index}
            className="bg-white rounded-2xl overflow-hidden shadow hover:shadow-lg transition text-left flex-shrink-0"
          >
            <div className="relative">
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 md:left-24 lg:left-32 md:translate-x-0 bg-yellow-500 text-gray-900 text-xs md:text-xs lg:text-sm px-3 md:px-3 lg:px-4 py-1.5 md:py-1.5 lg:py-2 rounded-t-lg rounded-b-none shadow border-t-2 md:border-t-3 lg:border-t-4 border-l-2 md:border-l-3 lg:border-l-4 border-r-2 md:border-r-3 lg:border-r-4 border-white">
                {blog.date}
              </div>
            </div>
            <div className="pt-6 md:pt-6 lg:pt-8 px-4 md:px-4 lg:px-5 pb-4 md:pb-4 lg:pb-5">
              <h3 className="font-semibold text-gray-900 text-base md:text-base lg:text-lg mb-2">
                {blog.title}
              </h3>
              <p className="text-xs md:text-xs lg:text-sm text-gray-500 mb-3">{blog.description}</p>
              <button
                onClick={(e) => handleBlogDetails(blog.slug, e)}
                className="text-green-900 text-xs md:text-xs lg:text-sm font-semibold hover:underline flex items-center"
              >
                Read More
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="ml-2 w-4 h-4 md:w-4 md:h-4 lg:w-5 lg:h-5 text-green-900"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
